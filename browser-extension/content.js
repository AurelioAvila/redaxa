...
// Tried auto-intercepting Enter-to-send before this (preventDefault +
// stopImmediatePropagation at document_start on window, the earliest
// possible point in the capture chain). Verified live on chatgpt.com that it
// does NOT reliably work: the raw, unredacted message still reached
// ChatGPT's backend before our dialog ever rendered, and the follow-up
// "Auto-redact and send" action then sent a *second* message on top of it --
// net effect was worse than doing nothing (the sensitive data still leaked,
// plus a duplicate message). ChatGPT's real submission isn't gated on the
// keydown event's default action/propagation in a way a content script can
// dependably beat, and that's before accounting for four other sites with
// their own different, undocumented internals. A "Check before you send"
// button the user presses is slower, but it never lies about whether
// something was actually sent -- which a broken auto-block would.
