...
export function mountDashboard(): void {
  ...
  const loadOrgActivity = async (): Promise<void> => {
    ...
    try {
      ...
      orgActivityList.innerHTML = events.map((event) => {
        const kinds = [...new Set(event.finding_kinds)].map((kind) => labels()[kind] ?? kind).join(", ");
