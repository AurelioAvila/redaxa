...
// layer speaks: policies match on categories and kinds, never on regexes.
export type FindingCategory = "personal" | "credentials" | "financial" | "custom";

// Severity is a property of what LEAKING this kind of data costs, decided
// here in the engine (single source of truth) rather than per-surface:
//  - critical: direct account/system compromise (keys, passwords, tokens)
//  - high:     direct financial exposure or strong identifiers (cards, SSN)
//  - medium:   personal identifiers with real but indirect harm (email, address)
//  - low:      weak identifiers, context-dependent (names, IPs)
...
const rules: Rule[] = [
  ...
  {
    kind: "address", category: "personal", severity: "medium", label: "Street address", replacement: "[ADDRESS]",
  ...
  },
  {
    ...
    pattern: /\b(?:sk-[A-Za-z0-9_-]{16,}|sk_(?:live|test)_[A-Za-z0-9]{10,}|pk_(?:live|test)_[A-Za-z0-9]{10,}|rk_live_[A-Za-z0-9]{10,}|gh[pousr]_[A-Za-z0-9_]{20,}|AIza[\w-]{20,}|AKIA[0-9A-Z]{16}|xox[baprs]-[A-Za-z0-9-]{10,}|eyJ[A-Za-z0-9_-]{10,}\.[A-Za-z0-9_-]{10,}\.[A-Za-z0-9_-]{10,}|Bearer\s+[A-Za-z0-9._-]{16,}|npm_[a-zA-Z0-9]{36}|pypi-AgEIcHlwaS5vcmc[a-zA-Z0-9\-_]{50,}|glpat-[a-zA-Z0-9\-]{20}|hf_[a-zA-Z0-9]{34})\b/g
  },
  ...
  { kind: "fiscalCode", category: "personal", severity: "high",  label: "Italian fiscal code", replacement: "[FISCAL CODE]", pattern: /\b[A-Z]{6}\d{2}[A-Z]\d{2}[A-Z]\d{3}[A-Z]\b/gi },
  ...
  { kind: "phone", category: "personal", severity: "medium",  label: "Phone number", replacement: "[PHONE]", pattern: /(?<![\w.])(?:\+?\d{1,3}[ -]?)?(?:\(?\d{2,4}\)?[ -]?)?\d{3,4}[ -]\d{3,4}(?!\w)(?!\.\d)/g, validate: phoneValid },
  { kind: "ip", category: "personal", severity: "low",  label: "IPv4 address", replacement: "[IP ADDRESS]", pattern: /\b(?:(?:25[0-5]|2[0-4]\d|1?\d?\d)\.){3}(?:25[0-5]|2[0-4]\d|1?\d?\d)\b/g },
  {
    kind: "credential", category: "credentials", severity: "critical", label: "Password or credential", replacement: "$1$2[REDACTED]",
...
export function inspectPrompt(text: string, options: ScanOptions = { includePersonalData: true, includeCredentials: true, includeFinancialData: true }): { findings: Finding[]; redactedText: string } {
  ...
  for (const rule of rules) {
    ...
    if (rule.validate) {
      ...
      found.forEach((value) => findings.push({ kind: rule.kind, category: rule.category, severity: rule.severity, label: rule.label, value, replacement: rule.replacement }));
